...
const iframeCache = new lruCache.LRUCache({
  max: 1,
  ttl: 1000
});
...
var getIframeScroll = (({
  ...
  if (isInIframe) {
    ...
    const viewportBox = cssBoxModel.getBox(iframe);
    const box = cssBoxModel.getBox(el);
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      const remove = selector => {
        ...
        elements.forEach(el => {
          !el ? process.env.NODE_ENV !== "production" ? invariant(false, 'Cannot unmount element as it is not set') : invariant(false) : void 0;
...
const dragHandleCache = new lruCache.LRUCache({
  max: 5000,
  ttl: 1000
});
function findDragHandle(contextId, draggableId) {
  ...
  if (!possible.length) {
    process.env.NODE_ENV !== "production" ? warning(`Unable to find any drag handles in the context "${contextId}" ${selector}`) : void 0;
    return null;
  }
...
function getDimension$1(descriptor, el, windowScroll = origin) {
  ...
  const originalClient = cssBoxModel.calculateBox(el.getBoundingClientRect(), computedStyles);
  let client = {
    ...originalClient
  };
  let page = cssBoxModel.withScroll(cssBoxModel.calculateBox(el.getBoundingClientRect(), computedStyles), windowScroll);
...
const getClient = (targetRef, closestScrollable, offset, transform) => {
  let base = cssBoxModel.getBox(targetRef);
  if (transform) {
